...
fn filter_l2() {
    ...
    let ef_search = 30;
    let knbn = 10;
    let vec_tosearch = (0..dim).into_iter().map(|_| rng.sample(unif)).collect::<Vec<f32>>();
    //
    // Create a sorted vector of ids
    // the ids in the vector will be used as a filter
    let filtered_hns = Hnsw::<f32, DistL2>::new(max_nb_connection, nb_elem, nb_layer, ef_c, DistL2{});
    let mut filter_vector: Vec<usize> = Vec::new();
    for i in 300..400 {
        filter_vector.push(i);
