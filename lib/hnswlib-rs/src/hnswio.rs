...
//!
//! Examples of dump and reload of structure Hnsw is given in the tests (see test_dump_reload,
//! reload_with_mmap)
use hnsw::*;
///
// datafile
// MAGICDATAP : u32
// dimension : usize!!
// The for each point the triplet: (MAGICDATAP, origin_id , dimension , array of values bson
// encoded) ( u32, u64, ....)
//
...
// io
use std::{
  fs::{File, OpenOptions}, io, io::{BufReader, BufWriter}, path::PathBuf
};
...
///
///  - datamap : a bool for mmap uasge. The data point can be reloaded via mmap of data file dump.
///    This can be useful when data points consist in large vectors (as in genomic sketching) as in
///    this case data needs more space than the graph.
///
///  - mmap_threshold : the number of itmes above which we use mmap. Default is 0, meaning always
///    use mmap data Can be useful for search speed in hnsw if we have part of data resident in
///    memory.
#[derive(Copy, Clone)]
...
impl ReloadOptions {
  ...
  /// set mmap threshold i.e : The maximum number of data that will be reloaded in memory by
  /// reading file dump, the other points will be mmapped. As the upper layers are the most
  /// frequently used, these points will be loaded in memory during reading, the others will be
  /// mmaped. See test *reload_with_mmap()*
...
// This structure will check existence of dumps of same name and generate a unique filename if
// necessary according to overwrite flag
#[allow(unused)]
pub struct DumpInit {
  // basename dump
  basename: String,
...
impl DumpInit {
  ...
  /// returns the basename used for the dump. May be it has been made unique to void overwriting a
  /// previous or mmapped dump
...
/// Note : **As this structure potentially contains the mmap data used in hnsw after reload it must
/// not be dropped before the reloaded hnsw.**
/// Example:
///
...
/// ```
///
/// In some cases we need a hnsw variable that can come from a reload **OR** a direct
/// initialization.
///
/// Hnswio must be defined before Hnsw as drop is done in reverse order of definition, and the
/// function [load_hnsw](Self::load_hnsw()) borrows Hnswio. (Hnswio stores the mmap address Hnsw can
/// refer to if mmap is used) It is also possible to preinitialize a Hnswio with the default()
/// function which leaves all the fields with blank values and use the function
/// [set_values](Self::set_values()) after. We get something like:
///
/// ```text
...
impl HnswIo {
  ...
  /// It is an error to call set_values on an already defined Hswnio by any function other than
    ...
    'a: 'b,
  {
    ...
    {
      // for all types except NoData , distance asked in reload declaration and distance in
      // dump must be equal!
    ...
    'a: 'b,
  {
    ...
    for l in 0..nb_layer as usize {
      ...
      for r in 0..nbpoints {
        // do we use mmap? for this point. We must load into memory up to threshold points,
        // and we also want the  most frequently accessed points, i.e those
        // in upper layers! to be physically loaded. So we do use mmap from
        // the moment the number of points yet to be loaded is less than threshold.
    ...
    let point_indexation = PointIndexation {
      ...
      nb_point: Arc::new(RwLock::new(nb_points_loaded)), /* CAVEAT , we should increase ,
                                                          * the whole thing is to be able
                                                          * to increment graph ? */
  ...
  T: 'static + DeserializeOwned + Clone + Sized + Send + Sync,
{
  ...
  if std::any::TypeId::of::<T>() != std::any::TypeId::of::<NoData>() {
    v = match descr.format_version {
      ...
      3 => {
        let slice_t = unsafe {
          std::slice::from_raw_parts(v_serialized.as_ptr() as *const T, descr.dimension as usize)
        };
        slice_t.to_vec()
      }
      _ => {
...
mod tests {
  ...
  fn test_dump_reload_1() {
    ...
    let hnsw =
      Hnsw::<f32, dist::DistL1>::new(nb_connection, nbcolumn, 16, ef_construct, dist::DistL1 {});
    for i in 0..data.len() {
  ...
  fn test_dump_reload_myfn() {
    ...
    let hnsw =
      Hnsw::<f32, dist::DistPtr<f32, f32>>::new(nb_connection, nbcolumn, 16, ef_construct, mydist);
    for i in 0..data.len() {
      hnsw.insert((&data[i], i));
    }
    // some loggin info
