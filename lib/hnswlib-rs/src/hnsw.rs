...
impl <'a,'b, T:Clone+Send+Sync+'b> Iterator for IterPointLayer<'a,'b, T> {
    ...
    //
    fn next(&mut self) -> Option<Self::Item> {
      return if (self.slot_in_layer) < self.pi_guard[self.layer].len() {
...
impl <'b, T:Clone+Send+Sync, D: Distance<T>+Send+Sync > Hnsw<'b, T,D>  {
    ...
    fn search_layer(& self, point: &[T], entry_point: Arc<Point<'b, T>> , ef:usize, layer: u8, filter:Option<& dyn FilterT>) -> BinaryHeap<Arc<PointWithOrder<'b, T>> > {
        ...
        while candidate_points.len() > 0 {
            ...
            trace!("comparaing c : {:?} f : {:?}", -c.dist_to_ref, f.dist_to_ref);
            ...
            trace!("       search_layer, {:?} has  nb neighbours  : {:?} ", c_pid, neighbours_c_l.len());
            for e in neighbours_c_l {
                ...
                if visited_point_id.contains_key(&e.point_ref.p_id) != true {
                    ...
                    if e_dist_to_p < f_dist_to_p || return_points.len() < ef {
                        ...
                        trace!("                inserting new candidate {:?}", e_prime.point_ref.p_id);
   ...
   pub fn insert_slice(&self, data_with_id: (&[T],usize))  {
        ...
        for l in ((level+1)..(max_level_observed+1)).rev() {
            ...
            trace!("in insert :search_layer layer {:?}, returned {:?} points ", l, sorted_points.len());
    ...
    fn reverse_update_neighborhood_simple(&self, new_point: Arc<Point<T>>) {
      //  println!("reverse update neighbourhood for  new point {:?} ", new_point.p_id);
        trace!("reverse update neighbourhood for  new point {:?} ", new_point.p_id);
        let level = new_point.p_id.0;
        for l in (0..level+1).rev() {
            for q in &new_point.neighbours.read()[l as usize] {
                if new_point.p_id != q.point_ref.p_id {
                    ...
                    let already = q_point_neighbours[l_n].iter().position(|old| old.point_ref.p_id == new_point.p_id);
                    if already.is_some() {
    ...
    fn select_neighbours(&self, data: &[T], candidates: &mut BinaryHeap<Arc<PointWithOrder<'b, T>> >,
        ...
        {
        //
        trace!("entering select_neighbours : nb candidates: {}", candidates.len());
        //
        ...
        if extend_candidates {
            ...
            trace!("select neighbours extend candidates from  : {:?} adding : {:?}", candidates.len(), new_candidates_set.len());
        ...
        if log::log_enabled!(log::Level::Trace) {
            trace!("exiting select_neighbours : nb candidates: {}", neighbours_vec.len());
