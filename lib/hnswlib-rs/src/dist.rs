...
unsafe fn distance_l1_f32<S: Simd>(va: &[f32], vb: &[f32]) -> f32 {
    ...
    for i in simd_length..va.len() {
        //        log::debug!("distance_l1_f32, i {:?} len {:?} nb_simd {:?} VF32_WIDTH {:?}", i,
        // va.len(), nb_simd, S::VF32_WIDTH);
        dist += (va[i] - vb[i]).abs();
    }
    assert!(dist >= 0.);
    dist
...
impl Distance<f32> for DistL1 {
    fn eval(&self, va: &[f32], vb: &[f32]) -> f32 {
        ...
        if #[cfg(feature = "simdeez_f")] {
            ...
            assert_eq!(va.len(), vb.len());
            unsafe {
                manhattan_similarity_neon(va, vb)
            }
        }
        else {
            va.iter().zip(vb.iter()).map(|t| (*t.0 as f32- *t.1 as f32).abs()).sum()
    ...
    fn eval(&self, va: &[f32], vb: &[f32]) -> f32 {
        ...
        #[cfg(target_feature = "neon")]
        {
            if is_aarch64_feature_detected!("neon") {
                // assert_eq!(va.len(), vb.len());
                return unsafe { euclid_similarity_neon(va, vb) };
            }
        }
...
impl Distance<f32> for DistDot {
    fn eval(&self, va: &[f32], vb: &[f32]) -> f32 {
        ...
        #[cfg(target_feature = "neon")]
        {
            if is_aarch64_feature_detected!("neon") {
                assert_eq!(va.len(), vb.len());
                return unsafe { dot_similarity_neon(va, vb) };
            }
        }

        let dot = va.iter().zip(vb).map(|t| (*t.0 * *t.1)).sum();
        dot
    }
}
